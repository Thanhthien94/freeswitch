...
export async function GET(request: NextRequest) {
  try {
    ...
    const fullUrl = `${backendUrl}/api/v1/freeswitch/network-config/status${queryString ? `?${queryString}` : ''}`
    const rawCookies = request.headers.get('cookie') || ''
    
    // Decode cookies to prevent double-encoding
    const cookies = decodeURIComponent(rawCookies)

    console.log('🔍 Frontend /api/freeswitch/network-config/status called')
    ...
    console.log('✅ FreeSWITCH network config status data received from backend')

    return NextResponse.json(data)
  } catch (error) {
    console.error('❌ FreeSWITCH network config status proxy error:', error)
    return NextResponse.json(
