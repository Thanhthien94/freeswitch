...
export async function GET(
  ...
  try {
    ...
    const fullUrl = `${backendUrl}/api/v1/freeswitch/extensions/${params.id}/registration${queryString ? `?${queryString}` : ''}`
    const rawCookies = request.headers.get('cookie') || ''
    
    // Decode cookies to prevent double-encoding
    const cookies = decodeURIComponent(rawCookies)

    console.log('🔍 Frontend /api/freeswitch/extensions/[id]/registration called')
    ...
    console.log('✅ FreeSWITCH extension registration data received from backend')

    return NextResponse.json(data)
  } catch (error) {
    console.error('❌ FreeSWITCH extension registration proxy error:', error)
    return NextResponse.json(
