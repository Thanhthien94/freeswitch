...
export async function GET(
  ...
  try {
    ...
    const fullUrl = `${backendUrl}/api/v1/freeswitch/extensions/${params.id}/xml${queryString ? `?${queryString}` : ''}`
    const rawCookies = request.headers.get('cookie') || ''
    
    // Decode cookies to prevent double-encoding
    const cookies = decodeURIComponent(rawCookies)

    console.log('🔍 Frontend /api/freeswitch/extensions/[id]/xml called')
    ...
    console.log('✅ FreeSWITCH extension XML data received from backend')

    return NextResponse.json(data)
  } catch (error) {
    console.error('❌ FreeSWITCH extension XML proxy error:', error)
    return NextResponse.json(
