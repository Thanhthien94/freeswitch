...
export async function GET(request: NextRequest) {
  try {
    ...
    const fullUrl = `${backendUrl}/api/v1/recordings/stats${queryString ? `?${queryString}` : ''}`
    const rawCookies = request.headers.get('cookie') || ''
    
    // Decode cookies to prevent double-encoding
    const cookies = decodeURIComponent(rawCookies)

    console.log('🔍 Frontend /api/recordings-stats called')
    ...
    console.log('✅ Recordings stats data received from backend')

    return NextResponse.json(data)
  } catch (error) {
    console.error('❌ Recordings stats proxy error:', error)
    return NextResponse.json(
