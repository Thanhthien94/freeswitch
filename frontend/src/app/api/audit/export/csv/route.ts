...
export async function GET(request: NextRequest) {
  try {
    ...
    const fullUrl = `${backendUrl}/api/v1/audit/export/csv${queryString ? `?${queryString}` : ''}`
    const rawCookies = request.headers.get('cookie') || ''
    
    // Decode cookies to prevent double-encoding
    const cookies = decodeURIComponent(rawCookies)

    console.log('🔍 Frontend /api/audit/export/csv called')
    ...
    console.log('✅ Audit export data received from backend')
