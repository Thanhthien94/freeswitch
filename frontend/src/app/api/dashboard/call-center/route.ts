...
export async function GET(request: NextRequest) {
  try {
    ...
    const fullUrl = `${backendUrl}/api/v1/dashboard/call-center${queryString ? `?${queryString}` : ''}`
    const rawCookies = request.headers.get('cookie') || ''
    
    // Decode cookies to prevent double-encoding
    const cookies = decodeURIComponent(rawCookies)

    console.log('🔍 Frontend /api/dashboard/call-center called')
    ...
    console.log('✅ Dashboard call-center data received from backend')

    return NextResponse.json(data)
  } catch (error) {
    console.error('❌ Dashboard call-center proxy error:', error)
    return NextResponse.json(
