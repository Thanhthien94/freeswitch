...
export async function GET(request: NextRequest) {
  try {
    ...
    const fullUrl = `${backendUrl}/api/v1/dashboard/system-status${queryString ? `?${queryString}` : ''}`
    const rawCookies = request.headers.get('cookie') || ''
    
    // Decode cookies to prevent double-encoding
    const cookies = decodeURIComponent(rawCookies)

    console.log('🔍 Frontend /api/dashboard/system-status called')
    ...
    console.log('✅ Dashboard system-status data received from backend')

    return NextResponse.json(data)
  } catch (error) {
    console.error('❌ Dashboard system-status proxy error:', error)
    return NextResponse.json(
